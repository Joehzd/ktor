...
//----------------------------------------------------------------------------------------------------------------------
// The implementation was copied from kotlinx.coroutines with Java 8 compatibility fixes (see "Java 8 compat")
...
internal class CoroutinesTimeoutExtension internal constructor(
    ...
    private fun <T> Class<T>.coroutinesTimeoutAnnotation(): Optional<CoroutinesTimeout> =
        AnnotationSupport.findAnnotation(this, CoroutinesTimeout::class.java).compatOr {
    ...
    private fun <T : Any?> interceptMethod(
        ...
        if (testAnnotationOptional.compatIsEmpty && classAnnotationOptional.compatIsEmpty) {
...
// START: Java 8 compat
private val Optional<*>.compatIsEmpty
    get() = !isPresent

private fun <T> Optional<T>.compatOr(supplier: () -> Optional<T>): Optional<T> =
    if (isPresent) this else supplier()
// END: Java 8 compat
